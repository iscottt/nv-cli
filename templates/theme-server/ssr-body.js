...
var get_footer = ()=>{
	var copyright = get_option("samsara_copyright", "PANDA Studio - nvPress")
	var links = get_option('samsara_footer_links',[]).map(item=>{
...
var render_articles = (next,{echo,req,route})=>{
	...
	var main_posts = query_posts({
					...
					status: 'publish',
					current_page,
					posts_per_page: get_option('samsara_posts_per_page',10)
				});
